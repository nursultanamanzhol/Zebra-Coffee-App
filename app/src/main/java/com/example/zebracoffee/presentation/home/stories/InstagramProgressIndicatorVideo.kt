...
@Composable
fun InstagramProgressIndicatorVideo(
    modifier: Modifier = Modifier,
    stepCount: Int,
    stepDuration: List<Long>,
    ...
    LaunchedEffect(isPaused, currentStep) {
        if (isPaused) {
            ...
            for ((index, duration) in stepDuration.withIndex()) {
                progress.animateTo(
                    targetValue = 1f,
                    animationSpec = tween(
                        durationMillis = ((1f - progress.value) * duration.toFloat()).toInt(),
                        easing = LinearEasing
                    )
                )
